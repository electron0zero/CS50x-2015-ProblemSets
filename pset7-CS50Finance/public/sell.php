<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        else
        {
            ...
            $rows = query("SELECT shares FROM shares WHERE id = ? and symbol = ?", $_SESSION["id"], strtoupper($_POST["code"]));
            if (count($rows) == 1)
            {
            ...
            $query = query("DELETE FROM shares where id = ? and symbol = ?", $_SESSION["id"], strtoupper($_POST["code"]));
            if ($query === false)
            {
    ...
    else
    {
        ...
        $rows = query("SELECT symbol FROM shares WHERE id = ?", $_SESSION["id"]);
        render("sell_templates.php", ["title" => "Sell", "symbols" => $rows]);
    }
?>
