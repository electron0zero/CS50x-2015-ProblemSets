...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            //getting things from request line 
            //request-line format : method<space>request-target<space>HTTP-version\r\n
            // variable line conatins request line so let's get things from there
            int linelen = 0;
            linelen = strlen(line);     //length of line
            
            int i=0;          
            char method[50];
            char req_target[500];
            char http_ver[20];
            int spA = 0,spB = 0,Qstart = 0;

            //traverse line and get data from it to validate
            for(i=0;i<linelen;i++)    {
                if(line[i]==' '){
                    //printf("got a space at i = %i \n",i);
                        
                    if(line[i]==' ' && spA == 0 ){
                        spA = i;  //position of first <space> in request-line
                        }
                    if( line[i]==' ' && spA != 0 ){
                        spB = i;  //position of second <space> in request-line
                        }
                    }
                    if(line[i]=='?' && spA != 0){
                        Qstart = i;     //query start index use it to seperate query and file extension from request-target 
                    }
            }
            //printf("flags = %i,%i & Qstart is %i",spA,spB,Qstart);
           
            //extracting method from line
            int a=0;
            for(a=0;a<spA;a++){
                method[a]=line[a];
            }
            ...
            //extracting request target from line
            int k=0,l=0;
            if(Qstart){
                    //request target with query
                    for(l=spA+1,k=0;l<Qstart;l++,k++){
                        req_target[k]=line[l];
                    }
                    req_target[k]='\0';
            }
            else{   //request target without query
                for(l=spA+1,k=0;l<spB;l++,k++){
                    req_target[k]=line[l];
                }
                req_target[k]='\0';
            }
                    ...
                    //printf("length:%d\n",strlen(req_target));
           //extracting http-version from line. 
           int hA=0;
           for(hA=spB+1;hA<linelen-2;hA++){
                http_ver[hA-(spB+1)]=line[hA];
           }
           http_ver[hA-(spB+1)]='\0';
            ...
            //checking for method if method is not GET return error 405 method not allowed
            if(strcasecmp("GET",method)!=0){
                error(405);
                //printf("broken : due to wrong method\n");
                continue;
            } 
            
            //checking request target that it starts of "/" if not throw 501
            //ASCII code for / is 47
            if(req_target[0]!=47){
            error(501);
            //printf("request target is messed up\n");
            continue;
            } 

            //if request-target contains a " , respond to the browser with 400 Bad Request
            //ASCII code for " is 34
            for(i=0;i<strlen(req_target);i++){
                if(req_target[i]==34){
                error(400);
                continue;
                //printf("got a \" in request target \n");
                }
            }

            //if HTTP-version is not HTTP/1.1, respond to the browser with 505 HTTP Version Not Supported
            if(strcasecmp("HTTP/1.1",http_ver)!=0){
               //printf("HTTP version is messed up\n");
                error(505);
                continue;
                
            }

            //if absolute-path(request target - query) does not contain a . (and thus a file extension), respond to the browser with 501 Not Implemented.
            //ASCII code for . is 46
            int validflag=1;
            for(i=0;i<strlen(req_target);i++){
                if(req_target[i]==46){
                //if . is found set validflag=0
                validflag=0;
                }    
            }
            if(validflag){
                //printf("there is no . in absolute path thus no file extension\n");               
                error(501);
                continue;               
                }
            //reqest line validation finished

            // TODO: extract query from request-target
            int qA=0;
            char query[100];
            if(Qstart){
                for(qA=Qstart+1;qA<spB;qA++){
                    query[qA-(Qstart+1)]=line[qA];
                }
                query[qA-(Qstart+1)]='\0';
                //printf("Query is :%s\n",query);
                //printf("length:%d\n",strlen(query));
            ...
            //overwrite path with with root then concatinate absolute path (req_target)
            char path[500];
            strcpy(path,root);
            strcat(path,req_target);
            //printf("path:%s\n",path);
           // printf("length:%d\n",strlen(path));

            // TODO: ensure path exists
            // F_OK mode for that if file exists it returns 0 otherwise check "man access" , respond with 404 if it dosen't
            if(access(path,F_OK)!=0){
                //printf("No such file exists, that file was a lie\n");
                error(404);
                continue;
            } 

            // TODO: ensure path is readable
            // R_OK mode for that if file is readable it returns 0 otherwise check "man access" , respond with 403 if it dosen't
            if(access(path,R_OK)!=0){
                //printf("file is not accasible, out of reach\n");
                error(403);
                continue;
            }  
            ...
            for(i=0;i<strlen(req_target);i++){
                if(req_target[i]==46){
                    extflag=1;  //when encounters a . in path extarct extension from there
                    loc=i;  //location if i when where we encountered a .
                }
            }
            if(extflag){    //if we got a . in path
                for(i=loc+1;i<strlen(req_target);i++){
                    extension[i-(loc+1)]=req_target[i];
                }
                extension[i-(loc+1)]='\0';
            }
            //printf("extension:%s\n",extension);
...
const char* lookup(const char* extension)
{
    ...
    //NULL otherwise.    // return MIME type of file
